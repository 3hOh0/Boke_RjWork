...
class AutosaveAPITests(TestCase):
    def setUp(self):
        ...
        self.article = Article.objects.create(title='A1', body='body', author=self.user, category=self.cat, article_order=0)

    def test_autosave_app_create_and_list(self):
        payload = {'article_id': self.article.id, 'title': 'Draft1', 'content': 'abc', 'version': 1}
        resp = self.client.post('/autosave/', data=json.dumps(payload), content_type='application/json')
    ...
    def test_blog_api_autosave_and_restore(self):
        ...
        resp = self.client.post('/api/autosave/draft/', data=json.dumps(payload), content_type='application/json')
        self.assertEqual(resp.status_code, 200)
        d = resp.json()
        ...
        for i in range(6):
            p = {'article_id': article_id, 'title': f'V{i}', 'body': f'body{i}', 'save_type': 'auto'}
            r = self.client.post(f'/api/autosave/draft/{article_id}/', data=json.dumps(p), content_type='application/json')
            self.assertEqual(r.status_code, 200)

        auto_count = ArticleDraft.objects.filter(article_id=article_id, save_type='auto').count()
