...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 7. 创建评论
        comments = self._create_comments(users, articles, options['comments'])
                ...
                if created:
                    ...
                    for article in selected_articles:
                        ...
                        item, item_created = models.FavoriteItem.objects.get_or_create(
                            ...
                            defaults={
                                'added_by': actor,
                                'note': f'收藏于 {created_time.strftime("%Y-%m-%d")}',
    ...
    def _create_comments(self, users, articles, count):
        """创建评论数据"""
        comments = []
        comment_bodies = [
            '写得很好，学到了很多！',
        ...
        for _ in range(count):
            ...
            # 随机分配过去30天内的时间
            days_ago = random.randint(0, 30)
            creation_time = timezone.now() - timedelta(days=days_ago, hours=random.randint(0, 23))
            ...
            comment = Comment.objects.create(
                ...
                last_modify_time=creation_time,
                is_enable=True,
            )
            comments.append(comment)
            
            # 10% 的概率创建回复评论
            if random.random() < 0.1 and len(comments) > 1:
                ...
                reply_time = creation_time + timedelta(hours=random.randint(1, 24))
